...
def is_move_okay?(first, second)
  ...
  is_okay = first.ord - second.ord <= 1
end
...
pointers = Set.new([finish])
visited = Set.new([finish])
steps_taken = 0

def is_lowest_point?(point, matrix)
  value = matrix[point[0], point[1]]
  lowest = value == "a" || value == "S"
  puts "value is a or S!" unless !lowest
  lowest
end

def contains_lowest_point?(points, matrix)
  points.any? {|point| is_lowest_point?(point, matrix)}
end

while (!contains_lowest_point?(visited, matrix)) do
  expansion = Set.new
